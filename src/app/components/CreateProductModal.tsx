...
export function CreateProductModal({ isOpen, onClose }: CreateProductModalProps) {
    ...
    try {
      ...
      if (uploadError) {
        ...
        if (uploadError.message.includes('not found') || uploadError.message.includes('does not exist')) {
          ...
          // A imagem já está em base64 no imagePreview
          return imagePreview;
        }
        throw uploadError;
      }
      ...
      // Usar a imagem em base64 como fallback
      return imagePreview;
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validar que tem imagem (arquivo ou URL)
    const temImagem = imageFile || imagePreview || formData.imageUrl.trim();
    if (!temImagem) {
      setUploadError('❌ Selecione uma imagem para o produto.');
    ...
    const loadingToastId = toast.loading('⏳ Salvando produto...');

    try {
      let imageUrl = '';

      // Prioridade: arquivo > URL direta > preview base64
      if (imageFile) {
    ...
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full sm:w-full md:max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl bg-white shadow-2xl ring-1 ring-[#E0B58C]/20">
        ...
        <div className="sticky top-0 z-20 flex items-center justify-between bg-white border-b border-[#F0E6DD] p-4 sm:p-6 rounded-t-3xl sm:rounded-t-3xl">
          <div>
            <h2 className="text-2xl sm:text-3xl font-serif font-bold text-[#2D160C]">
              Novo Produto
            </h2>
